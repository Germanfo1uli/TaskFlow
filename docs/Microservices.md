...
#### Каждый микросервис — отдельный сервис (Spring Boot или .NET), со своей схемой БД (можно в одной физической PostgreSQL, но с префиксами схем, например users_, board_, и т.д.). Для каждого отдельного микросервиса в отдельных сущностях реализуются методы CRUD.

#### Для взаимодействия со всеми микросервисами будет создан API Gateway (Инструмент пока хз), настроен докер для запуска микросервисов, а также автоматическая миграция базы данных.
...
#### Пример для .NET:
```"ConnectionStrings": {
    "Default": "Host=pg-primary;Database=jira_db;Username=issues_service;Password=xxx;SearchPath=issues_"
}
```
